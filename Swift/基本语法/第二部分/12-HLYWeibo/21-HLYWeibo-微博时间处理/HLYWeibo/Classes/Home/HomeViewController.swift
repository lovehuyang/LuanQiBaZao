...
class HomeViewController: BaseViewController {
    ...
    private lazy var statusArr : [Status] = [Status]()// 存放微博数据的数组
...
extension HomeViewController{
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        //2、 给cell设置数据
        let status : Status = statusArr[indexPath.row]
        cell.textLabel?.text = status.sourceText
        cell.detailTextLabel?.text = NSDate.createDateStr(createAtStr: status.created_at!)
