...
class BaseViewController: UITableViewController {
    ...
    override func loadView() {
        // 1 从沙盒中读取归档的信息
        // 1.1获取沙盒路径
        let accountPath = "\(NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first! as String)/account.plist"
        // 1.2 解档
        let account = NSKeyedUnarchiver.unarchiveObject(withFile: accountPath) as? UserCount
       
        if account != nil {
            // 1.3取出过期日期 和当前日期对比
            let expiresData = Date.init(timeIntervalSince1970: "157679999" as! TimeInterval)
