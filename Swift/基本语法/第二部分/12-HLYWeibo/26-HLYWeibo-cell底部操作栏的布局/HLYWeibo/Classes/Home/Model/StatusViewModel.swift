...
class StatusViewModel: NSObject {
    ...
    init(status : Status) {
        ...
        if status.source != nil, status.source != "" {
            ...
            sourceText = "来自\((status.source! as NSString).substring(with: NSRange.init(location: startIndex, length: length)))"
        }
        ...
        profileURL = NSURL(string: profileStr)! as URL
